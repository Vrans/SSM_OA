...
<%@ page import="com.vran.oa.util.Contant" %>
...
<div class="section row">
      <C:forEach items="${items}" var="item">
            <div class="col-md-3">方式：${item.item}</div>
            <div class="col-md-3">金额：${item.amount}</div>
            <div class="col-md-5">事由：${item.comment}</div>
      </C:forEach>
</div>
<div class="section row">
...
<div class="section row">
      <c:forEach items="${records}" var="record">
            ...
            <div class="col-md-3"><spring:eval expression="record.dealTime"/>
            </div>
                  ...
                  <div class="panel-footer text-right">
                        <c:if test="${sessionScope.employee.post==Contant.POST_FM || sessionScope.employee.post==Contant.POST_GM}">
                              <button type="submit" class="button" name="dealWay"
                                      value="${Contant.DEAL_PAST}">${Contant.DEAL_PAST}</button>
                              <button type="submit" class="button" name="dealWay"
